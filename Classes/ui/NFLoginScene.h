...
//    void OnLoadRecCallBack(CCObject *pObj);		//回调：资源加载
    ...
    //---------------登录页按钮回调----------------
    void OnBtnEnterCallBack(Ref *pSender, Widget::TouchEventType type);			//开始游戏
    //选择服务器
    void OnBtnServerSelectCallBack(Ref* pSender, Widget::TouchEventType type);
    
    void OnBtnSwitchToRegisterCallBack(Ref *pSender, Widget::TouchEventType type);	//跳转到注册页
    void OnBtnLoginCallBack(Ref *pSender, Widget::TouchEventType type);			//登录
    void OnAccountTextFieldEvent(Ref *pSender, TextFiledEventType type);
    
    
    void OnBtnServerSelectCallBack2(float fg);
